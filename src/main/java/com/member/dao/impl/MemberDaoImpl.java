...
//		(Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(sql))
		{
			final MemberVO oldMember = getSession().load(MemberVO.class, member.getMember_id());
	...
	public MemberVO findByPrimaryKey(Integer id) {
		...
		try 
		(Connection connection = dataSource.getConnection(); PreparedStatement ps = connection.prepareStatement(sql)) {
			ps.setInt(1, id);
